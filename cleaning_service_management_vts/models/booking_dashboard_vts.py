...
class BookingDashboard(models.Model):
    ...
    def _get_period_domain(self, period='month'):
        ...
        return [('preferred_date', '>=', start_date)]
    ...
    def get_service_analysis(self, period='month'):
        ...
        return [
            {
                'service': d['service_type_id'][1],
                'count': d['service_type_id_count'],
            }
    ...
    def get_top_cleaners(self, period='month'):
        ...
        sorted_cleaners = sorted(cleaners.items(), key=lambda x: x[1], reverse=True)[:5]
